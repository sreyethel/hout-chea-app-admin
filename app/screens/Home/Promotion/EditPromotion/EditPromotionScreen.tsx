...
export default class EditPromotionScreen extends React.Component<AppProps, any> {
	...
	componentDidMount() {
		this.setState({ discount: this.props.discount, date: this.props.date })
	}
	public render() {
			...
			<View>
				...
				<View style={styles.formGroups}>
					...
					<View style={styles.listView}>
						<ListAddProduct title="Product" icon={`inbox`} onPress={() => Alert.alert("Cannot adjust product!")} img={dataSelectedItem?.cover}>
							{dataSelectedItem ? <Text>{dataSelectedItem.name}</Text> : null}
						</ListAddProduct>
						<ListAddProduct title="Stock" icon={`dns`} onPress={() => Alert.alert("Cannot adjust stock!")}>
